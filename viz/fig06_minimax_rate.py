"""Deterministic diagnostic for the fixed-corner minimax theorem.

The left panel evaluates the comparable-pair probability p(tau) by
Gauss--Legendre quadrature.  The right panel displays the two n^{-1/2} risk
curves from Corollary 3.10.  Its lower-curve coefficient uses the maximum of a
finite tau mesh of direct-score Fisher evaluations and is therefore labelled
as a numerical diagnostic, not as a certified value of Ibar.

No sprinkling, validation seed, or sealed-instrument output is used.

Run:
    python3 viz/fig06_minimax_rate.py
"""
...
def comparable_probability(tau, quadrature_nodes=96):
...
def direct_fisher(tau, nodes=SCORE_NODES):
...
def draw(out):
    ...
    fisher_values = np.array([direct_fisher(tau)["fisher"] for tau in taus])
    ...
    fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(13.0, 5.1))
    ...
    ax_b.loglog(sample_sizes, plugin_upper, color=ORANGE, linewidth=2.4,
                label=r"plug-in theorem curve $2/(\kappa\Delta v\sqrt{n})$")
    ax_b.loglog(sample_sizes, le_cam_diagnostic, color=BLUE, linewidth=2.4,
                label=r"Le Cam curve with $\widehat{\bar I}_{\rm mesh}$ inserted")
    ax_b.loglog(sample_sizes, 0.2 / np.sqrt(sample_sizes), color=GREY,
                linestyle=":", linewidth=1.5, label=r"$n^{-1/2}$ guide")
    ax_b.set_xlabel(r"cardinality $n$")
    ax_b.set_ylabel("absolute-risk scale")
    ax_b.set_title(r"B · matching exponent, unresolved constant gap", loc="left")
    ax_b.grid(alpha=0.22, which="both")
    ax_b.legend(frameon=False, fontsize=8.8)
    ax_b.text(
        0.03,
        0.04,
        rf"$\widehat{{\bar I}}_{{\rm mesh}}={fisher_mesh_max:.6g}$ at $\tau={fisher_argmax:.3f}$"
        "\nmesh maximum is diagnostic, not a certified supremum",
        transform=ax_b.transAxes,
        fontsize=9.2,
        color=RED,
    )

    fig.suptitle("Fixed-corner shape information and the minimax rate", fontsize=14)
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    fig.savefig(out, dpi=180)
    plt.close(fig)
    return out, p_values, fisher_values, fisher_mesh_max, fisher_argmax, cauchy_ratio_min


if __name__ == "__main__":
    diagnostic_sweep()
    target = pathlib.Path(__file__).parent / "output" / "fig06_minimax_rate.png"
    target.parent.mkdir(exist_ok=True)
    path, p_values, fisher_values, fisher_mesh_max, fisher_argmax, cauchy_ratio_min = draw(target)
