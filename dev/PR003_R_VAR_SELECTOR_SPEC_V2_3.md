# PR-003 — R-VAR spec v2.3: selector variacional de consenso con abstención tipada (parche documental Parte E, comité 020)
...
sellado ni tracks de producción/parches BH. Escrita en cumplimiento de
`docs/comite/comite_decision_020_rvar-partF-degeneracy-disposition.md` §9 paso 5 (re-scope de
Parte E), autorizada explícitamente por el PI como enmienda de texto, secuenciada después del
paso 4 (disposition-commit de Part F, `docs/rvar_partF_disposition.md`, commit `896ec3e`).
...
1. D.2.1/D.2.2: el sketch de "DP local sobre pasos de escalera" queda **superseded** por la
   ...
   bruta con cero discrepancias en los 4 pasos de la iteración Dinkelbach. (Heredado de v2.2, sin
...
2. El hallazgo del empate espurio (`D=∅`, `D=C` empatan trivialmente con el óptimo real bajo la
   ...
   optimización, no heurísticas de limpieza posteriores. (Heredado de v2.2, sin cambio.)
3. Gate 0 (D.2.3) se mantiene intacto y sigue siendo obligatorio antes de cualquier cómputo de
   ...
   redundante. (Heredado de v2.2, sin cambio.)
4. **Parte E (ÚNICO cambio sustantivo de v2.3):** se retira la afirmación universal "se computa
   en tiempo polinomial para todo poset finito" — falsa como estaba escrita (comité 019/020: la
   única implementación verificada en Gate 0, `family_A`, enumera `2^|Max(C)|`; el min-cut de
   Picard resuelve el cierre NO restringido, no el selector restringido a `𝒜(C)`). Se sustituye
   por la afirmación acotada: computable en tiempo polinomial para órdenes finitos que superan el
   certificado de intervalo por-draw bajo un ordenamiento de antichain order-only y congelado;
   abstención tipada en caso contrario. La cláusula de no-cap-de-presupuesto NO se toca. El lema
   de intervalo se etiqueta explícitamente como derivado del proyecto (sin fuente en biblioteca).

**Esta revisión v2.3 NO autoriza Tier 1, ni el cómputo de μ, ni la continuación del candidato
interval-DP (que permanece DISQUALIFIED por leakage de clave, comité 020 §8(2)), ni ningún paso
de S1-S5 más allá de lo ya ejecutado en Gate 0 Tier 0.** Sustituye a
`dev/PR003_R_VAR_SELECTOR_SPEC_V2_2.md` (v2.2), que queda como registro histórico junto con
v2.1, v2 y v1; donde v2.2 y v2.3 difieren, **v2.3 gobierna**. v2.3 toca ÚNICAMENTE Parte E; todo
lo demás (incluido D.2.1/D.2.2, patchado en v2.2) se hereda sin cambio.
...
# Parte E — Nota de computabilidad (revisada v2.3 — over-claim corregido, comité 020)

**[Simplificación v2.2, PARCIALMENTE SUPERSEDED en v2.3 — ver corrección abajo]:** el algoritmo
de min-cut de D.2.1 (cierre de peso máximo, Picard 1976) es polinomial en `|C|` y `|coberturas|`
para **cualquier poset finito**, sin requerir `dim_DM(C)≤2` ni un realizador — a diferencia del
sketch de "DP sobre escalera" que sí dependía de esa estructura 2D. Esto sigue siendo cierto y no
se retira: es una mejora genuina descubierta al implementar Gate 0, y la condición `dim_DM≤2`
(que comité 012 D2 exige **solo para 𝔄_Schw**, la familia generadora, por conformidad plana 2D)
ya no es una condición de computabilidad **del subproblema de cierre no restringido**. ~~R-VAR
era, es, y sigue siendo definido por `≤` para todo poset finito, y ahora también se **computa**
en tiempo polinomial para todo poset finito, no solo para la subclase 2D.~~ **Esta última frase
queda SUPERSEDED — es falsa como estaba escrita** (comité 019, confirmado por comité 020 §4
mathematician/logician/falsifier): la única implementación verificada en Gate 0 (`family_A`,
`dev/measure_pr003_rvar_gate0.py:67-88`) restringe el óptimo a `𝒜(C)` — los down-sets `D=↓M` con
`M⊆Max` que además superan los filtros de admisibilidad `∅≠D≠C`, `(C−D)∩Min≠∅`, `H≠∅` — y esa
restricción se implementa por enumeración `2^|Max(C)|`, no por el min-cut solo; la sonda de
factibilidad (`014d364`) midió esa enumeración como `INFEASIBLE` a escala de producción
(`|Max|=40` en la intensidad más alta). El min-cut de Picard resuelve el cierre de peso máximo
**no restringido**; NO se ha verificado que resuelva el problema restringido a `𝒜(C)` para todo
poset finito.

**[FIX comité 020 §9 paso 5 — afirmación acotada, sustituye a la anterior]:** R-VAR es, y sigue
siendo, definido por `≤` para todo poset finito (Parte D.1, sin cambio). Se **computa** en
tiempo polinomial **para órdenes finitos que superan el certificado de intervalo por-draw**
(`I_v` contiguo en el antichain `Max` bajo un ordenamiento fijo — `dev/rvar_structure_probe_result.json`,
0 violaciones en 24/24 draws bajo el sort ingoing `p=t+r`; el sort outgoing `q=t−r*` FALLA el
certificado dentro de BH, confirmando que la propiedad depende del ordenamiento elegido, no es
universal) **bajo un ordenamiento de antichain order-only, derivado del orden y congelado antes
de su uso** (esa derivación está sin especificar — ver candidato interval-DP abajo); con
**abstención tipada en caso contrario** (certificado falla → no se computa, no se adivina). Para
órdenes finitos generales que no superan el certificado, la computabilidad polinomial del
problema restringido a `𝒜(C)` permanece **abierta** (comité 020 §5 falsifier: "puede que no
exista ningún algoritmo polinomial", D.1 es disyuntivo, no expresable como cierre de Picard sin
más estructura). El **lema de intervalo se etiqueta explícitamente como derivado del proyecto**:
no tiene fuente en `biblioteca/` (comité 020 §7, barrido de "suffix" en los 56 PDFs, cero hits) —
es una observación empírica sobre estas sprinklings 1+1D, apoyada en que el orden causal de
Minkowski 2D es de dimensión-orden 2 con las coordenadas nulas como realizador exacto (Bombelli
1987, `biblioteca/derived-md/Bombelli_1987_PhD.md`, CONFIRMADO), no un teorema citado de la
literatura. El candidato algorítmico que alcanzaría esta afirmación acotada
(`dev/explore_rvar_interval_dp.py`, gap-DP O(K²) sobre el certificado de intervalo) está **hoy
DISQUALIFIED para cualquier uso de scoring**: la sonda cross-key (`5014a39`,
`dev/rvar_crosskey_probe_result.json`, rama (a)) confirmó que su clave de ordenamiento depende de
la etiqueta oculta BH/MINK — fuga de ground-truth — y no existe todavía una derivación order-only
de esa clave. Ninguna afirmación de esta Parte E autoriza su uso hasta que las cinco
precondiciones de comité 020 §8(2) se satisfagan.

**Ningún cap de presupuesto puede introducirse en sprinkling sin reabrir la herida de censura
documentada en `dev/measure_pr003.py` (wounds #1/#2)** — esta cláusula no se toca por esta
revisión (comité 020 §9 forbidden-moves (3)(ii): cualquier cambio de política de cap de
presupuesto es una decisión de comité separada, no un efecto colateral de este re-scope).
...
```text
DOCUMENT_ID = PR003_R_VAR_SELECTOR_SPEC_V2_3
SUPERSEDES = PR003_R_VAR_SELECTOR_SPEC (v1), PR003_R_VAR_SELECTOR_SPEC_V2 (v2),
  PR003_R_VAR_SELECTOR_SPEC_V2_1 (v2.1), y PR003_R_VAR_SELECTOR_SPEC_V2_2 (v2.2); los cuatro
  conservados como registro histórico
PATCH_SCOPE = documental mínimo, motivado por
  docs/comite/comite_decision_020_rvar-partF-degeneracy-disposition.md §9 paso 5 — toca
  ÚNICAMENTE Parte E; el resto de v2.2 (D.2.1/D.2.2/D.2.3, cierres a-i, CIRCULARITY_STANDARD,
  V.1a/V.1b) se hereda sin cambio
CANDIDATE_PATH = C1 vía R directa (Q_DISPOSITION sin cambio)
REVISION_MANDATE = docs/comite/comite_decision_020_rvar-partF-degeneracy-disposition.md §9 paso
  5 (committing step, autorizado explícitamente por el PI, secuenciado después del paso 4 —
  ver docs/rvar_partF_disposition.md, commit 896ec3e)
...
CLOSURES_DELIVERED (re-verificadas por comité 017, corregidas donde el falsifier encontró que
  ...
  (j) [FIX comité 020 §9 paso 5] Parte E: retirada la afirmación universal "se computa en
      tiempo polinomial para todo poset finito" (falsa como estaba escrita — la única
      implementación Gate-0-verificada, family_A, enumera 2^|Max(C)|, medido INFEASIBLE a
      escala de producción en 014d364); sustituida por "computable en tiempo polinomial para
      órdenes finitos que superan el certificado de intervalo por-draw bajo un ordenamiento de
      antichain order-only y congelado; abstención tipada en caso contrario"; cláusula de
      no-cap-de-presupuesto sin tocar; lema de intervalo etiquetado como derivado del proyecto,
      sin fuente en biblioteca (comité 020 §7)              [RE-SCOPE CERRADO — computabilidad
      del problema restringido a 𝒜(C) para posets generales permanece ABIERTA, no una promesa
