#!/usr/bin/env python3
"""Prepare and optionally run external DeepMath consultations."""
...
DEFAULT_DEEPMATH_HOME = Path("~/ai/deepmath").expanduser()
DEFAULT_OUTPUT_ROOT = Path("dev/consultations/deepmath")
DEFAULT_TIMEOUT_SECONDS = 600
ADVISORY_LABEL = "ADVISORY_ONLY_NOT_EVIDENCE"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare an external DeepMath consultation dossier.")
    ...
    parser.add_argument("--deepmath-home", type=Path, default=Path(os.environ.get("DEEPMATH_HOME", DEFAULT_DEEPMATH_HOME)))
    parser.add_argument("--deepmath-cmd", default=os.environ.get("DEEPMATH_CMD"))
    parser.add_argument("--run", action="store_true", help="Run the configured external command.")
    parser.add_argument("--check-only", action="store_true", help="Only print DeepMath adapter status.")
...
def main() -> int:
    args = parse_args()
    repo_root = Path.cwd()
    status = inspect_deepmath(args.deepmath_home.expanduser(), args.deepmath_cmd)
    ...
    if not args.run:
        ...
        print("DeepMath not invoked: pass --run with DEEPMATH_CMD or --deepmath-cmd to execute.")
        return 0

    if not args.deepmath_cmd:
        write_text(output_dir / "response.md", render_failure("DEEPMATH_CMD is not configured.", status))
        print("error: DEEPMATH_CMD is not configured; response.md records the failure", file=sys.stderr)
        return 2

    command = command_argv(args.deepmath_cmd)
    if not command:
        write_text(output_dir / "response.md", render_failure("DeepMath command is empty.", status))
        print("error: DeepMath command is empty; response.md records the failure", file=sys.stderr)
        return 2

    if not command_exists(command[0]):
        message = f"DeepMath command not found: {command[0]}"
...
def inspect_deepmath(deepmath_home: Path, deepmath_cmd: str | None) -> dict[str, object]:
    argv = command_argv(deepmath_cmd) if deepmath_cmd else []
    return {
        "deepmath_home": str(deepmath_home),
        "deepmath_home_exists": deepmath_home.exists(),
        "deepmath_cmd_configured": bool(deepmath_cmd),
        "deepmath_cmd": deepmath_cmd,
        "deepmath_command_found": command_exists(argv[0]) if argv else False,
        "external_repo": external_repo_metadata(deepmath_home),
...
def render_failure(message: str, status: dict[str, object]) -> str:
    return "\n".join(
        [
            "# DeepMath consultation response",
            "",
