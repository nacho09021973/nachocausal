...
Throughout, S1 denotes the specific setup studied here: a flat $1+1$-dimensional causal diamond in null coordinates, carrying the product causal order and a uniform reference measure, perturbed by volume-normalized conformal exponentials, and observed only through the unlabeled causal order of a sprinkling conditioned on its cardinality. §2 makes this precise.

In this model, which components of a conformal perturbation remain visible after passing from the continuous copula to the unlabeled causal-order law at fixed cardinality $N$?
...
What remains is to sum the permutation-level differential over these fibers and ask what the class sums span. For every $N\ge2$ they span the full symmetric target module,
...
equivalently $T_N(\operatorname{im}J_N)=\operatorname{Sym}(E_N)$. Theorem 1 of §4 proves this constructively at every $N$, enumerating no posets. From the span follow the exact kernel, the identifiable quotient, and the factorization $D\mathscr S_N=B_NP_N^{\rm vis}$ with $F_N=B_N^*B_N$ positive definite on $V_N$ (§5), together with strict nesting $V_N\subsetneq V_{N+1}$ and density of $\bigcup_NV_N$ in the symmetric sector.

The span settles which directions survive, not how strongly the law encodes them. In the fixed symmetric Hilbert–Schmidt sector Fisher information is asymptotically retained: normalized by $N$ continuous copula observations, $\widehat F_N\to\Pi_{\rm sym}$ in the strong operator topology, and not in norm (§6). One explicit antisymmetric orbit has $r_N(\gamma_\psi)=2$ for every $N\ge2$ — invisible at first order, detected at second (§7) — an existence statement for a single witness, not a classification of the antisymmetric sector.

All of this concerns the S1 model in a $1+1$-dimensional causal diamond, at first order, or second order in §7, about the independent reference point. No statement here concerns higher dimensions, general Lorentzian spacetimes, Schwarzschild or horizons, nonlinear identifiability at finite distance, or reconstruction of a geometry from a causet; §9 records the limits precisely.
...
It follows from the likelihood, and assumes no independence after conditioning.
...
**Resolution inside the visible sector.** The projection $P_N^{\rm vis}$ records *which* directions survive. Their weight in the law is a second datum,
...
The corollary is an existence statement for one admissible orbit: it neither classifies the second differential on $\bigwedge^2H$ nor asserts that every antisymmetric direction has order two, and no general second-order operator, quadratic null cone, estimator or rate is introduced. The first-order zero is the exact isometric fold $\varepsilon\leftrightarrow-\varepsilon$ of (7.3), so membership in $\ker D\mathscr S_N$ does not imply invariance of the full nonlinear law.

## 8. Relation to previous work

The framework is [Bombelli2000]'s: the complete law of an unlabeled causal poset at fixed cardinality, together with a statistical comparison of two such laws. Around it, [Janson2011] supplies the limiting theory of poset kernels and consistent finite laws, while [Surya2026] shows that increasing resolution lifts degeneracies — though through expected interval abundances rather than the full unlabeled-poset law. None of them computes the differential of the finite-$N$ law, its rank, or its kernel.
