...
> **STATUS: LITERAL_INVENTORY / WORK_DATE_2026-07-29 / NOT_A_COMITE_ACTA / NO_NEW_MATHEMATICS /
> NO_CODE / NO_SIMULATION / NO_SEEDS / SEALED_PATH_UNTOUCHED.**
>
...
> independiente. Este documento no promueve, no re-etiqueta y no edita ningún resultado; registra
> qué hay, dónde está, y con qué estatus textual exacto.
...
Los ocho candidatos señalados por el PI fueron localizados todos; la revisión añade dos entradas
que el listado del PI no incluía (núcleo 2 y el ítem calificado Q2).

## 1. Resultado del conteo

```text
NUCLEOS_RANGO_TEOREMA_PLENO = 8
ITEMS_CALIFICADOS_NO_CONTADOS_COMO_PLENOS = 2
```

## 2. Los ocho núcleos plenos
...
- **Residencia anómala:** vive en `dev/` (nota matemática committeada) y **no está incorporado al
  manuscrito** — la única mención tangencial de maximalidad es `docs/manuscript_limits_draft.md:858`.
...
- **Dónde:** `docs/manuscript_limits_draft.md:365-402` (`[PROVED]`); prueba completa en
...
- **Dónde:** `docs/manuscript_limits_draft.md:455-489` (`[PROVED]`, etiqueta
...
- **Dónde:** `docs/manuscript_limits_draft.md:566-621` (`[PROVED]`); anexo
...
- **Sobre la parte delicada señalada por el PI (el canal de clases de isomorfismo):** está
  ...
  el manuscrito lo declara (:609). Finitud de Ī probada; su valor numérico `[NUMERICAL]`.
- **Caveat obligatorio del manuscrito (:615-621):** prohibido narrarlo como "cota de detector de
...
- **Dónde:** `research_program/work_packages/wp4_comparable_pair_separation.md:192-258`.
- **Caveats literales:** el orden `O(dv²)` de la corrección de C9 está apoyado en evidencia
  ...
  dos pasos bajo el Corolario C6 están "argued rather than written out" (:174-183) y `dv_0` no
  es efectivo (:210-212). El límite `1/36` en sí es exacto y probado.
...
## 3. Los dos ítems calificados (no contados como plenos)

### Q1 — Separación `fixed_n`: `TV → 1` vía Corolario C6 + varianza de Hoeffding + Chebyshev

La intuición del PI ("quizá todavía no esté promovido formalmente a teorema") es **exactamente
correcta en el texto**: el acta 045 lo adjudicó como *"teorema con UN hueco admitido — no es un
teorema desnudo y no debe llevar un `[PROVED]` desnudo"*
(`docs/comite/comite_decision_045_candidate-7-1-fixed-n-logical-status.md:192`). El hueco único
es el paso (i) del Anexo C §4 (cota de resto `O(dv²)` uniforme en τ, "argued, not written"); la
etiqueta adjudicada es `PROVED (leading order) / NON_EFFECTIVE_x2 / TWO_POINT_ONLY /
DIAMOND_FAMILY_ONLY` (:216-218), y el manuscrito lo mantiene deliberadamente fuera de los claims
("Conditional… omitted from claims until then", `docs/manuscript_limits_draft.md:1060`, pendiente
de las condiciones C3-C4 del acta 045). **Un solo paso analítico escrito** (la cota de resto) lo
promovería a núcleo pleno y haría efectivos `dv_0` y `n_0` simultáneamente (acta 045, (h)).
Nota de valor: es el único resultado del corpus en dirección **positiva** de separación
(consistencia de test a dos puntos), complementario de los suelos.
...
**Sí — con una precisión.** Los ocho núcleos no son bloqueos dispersos: forman una familia con un
...
| Cuantitativa: lo que sí se ve, se ve a tasa `√n` | 4, 5, Q1 | suelo minimax (todo estimador) **y** separación positiva (dos puntos), con exponente probado por ambos lados (acta 045 §(203)) |
...
consolidación firmada en el acta 046) contiene los núcleos 2, 3, 5 como sus Teoremas 3.1/3.2/3.8.
...
+ ledger de fracasos"; el inventario muestra que cuatro núcleos plenos (1, 6, 7, 8) y el ítem Q1
están **fuera** de él, y que con ellos dentro la contribución dominante es una familia coherente
...
4. Ítem Q1 → núcleo 9 potencial: **el paso analítico de mayor valor del corpus** es escribir la
   cota de resto `O(dv²)` uniforme (acta 045 lo identifica como el único ingrediente que falta,
   :170); cerraría C3-C4 y levantaría la restricción "omitted from claims" del manuscrito.
5. Núcleos 2, 6: anclar a fuente primaria de GR/estadística donde el texto lo pide; `biblioteca/`
