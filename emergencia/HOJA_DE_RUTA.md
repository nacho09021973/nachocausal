...
## 22. Reconciliación documental posterior al 6 de agosto (2026-08-16)

Esta sección reconcilia la cronología posterior a §21 con las reglas de alcance ya
vigentes. No reabre ninguna línea, no autoriza ejecuciones y no eleva ningún token
científico. Sus anclas son `docs/hoja_de_ruta_agosto_2026.md`,
`docs/backlog_hallazgos.md`, `docs/foro/foro_decision_001_ef4-falsacion-adversarial.md`,
`docs/scope_note_2026-08-16_foro001_falsification_test.md`,
`docs/c1_correction_2026-08-16_realizability.md` y
`docs/program_reopening_note_2026-07-31.md`.

### 22.1 Procedencia y alcance de EF-0..EF-8

La línea EF-0..EF-8 fue trabajo real posterior al 6 de agosto, desarrollado en la
rama `research/f2-f3-chain-distance`. La primera hoja EF se registró el 2026-08-11
como trabajo sobre la ley condicionada de `Z_n=sqrt(KL)/(n+1)` para `COUNT_VOLUME` en
`fixed-n`, `d=2`.

Eso no equivale a una reapertura válida del programa cerrado. La nota de reapertura
del 2026-07-31 fijaba un perímetro cerrado —R1 y R2— y la regla «Nada entra sin una
nueva nota firmada»; R3 fue añadido después mediante su propia nota firmada. La hoja
EF no sustituyó esa regla ni constituyó por sí sola una nota de reapertura del mismo
rango.

El 2026-08-15 el PI resolvió C-2 trasladando íntegramente EF-0..EF-8 al backlog. El
estado vigente es por tanto histórico y fuera de perímetro: conservar el material no
lo valida matemáticamente ni lo convierte en trabajo activo.

```text
EF0_EF8_PROVENANCE = REAL_POST_2026_08_06_BRANCH_WORK
EF0_EF8_REOPENING_STATUS = NOT_A_VALID_PROGRAM_REOPENING
EF0_EF8_CURRENT_SCOPE = BACKLOG_OUT_OF_SCOPE
EF0_EF8_EXECUTION_AUTHORIZATION = NO
```

### 22.2 Relación científica entre Ruta B, EF-4 y el blanco normalizado

EF-4 reutiliza y reformula la arquitectura combinatoria de la Ruta B —familia
prescrita, coste subexponencial de selección y control de rivales—, pero EF-4.3 no es
una simple copia literal del Lema 13.11. El lema antiguo quedó
`PROVED_MODULO_13.4`; EF-4 escribe una clasificación posterior con umbral `tau_n`,
topes explícitos de puntos prescritos y un margen corregido para el caso de pérdida.
Por ello cualquier auditoría de EF-4 debe hacerse sobre sus propias hipótesis y no
identificando automáticamente EF-4.3 con el texto antiguo.

La Advertencia 13.16 sigue siendo vinculante para la interpretación. El objetivo
científicamente informativo no es que el error absoluto desaparezca, sino el cociente
normalizado
...
Demostrar `P_{2,n}->0` puede ser compatible con `liminf T_n>0` y, por sí solo, no
establece recuperabilidad. EF-4 no contradice esta advertencia: su propio techo de
afirmación es el MSE absoluto del estimando lateral relativo y deja explícitamente
sin decidir la razón de información normalizada.

La consecuencia estratégica es precisa. Una futura auditoría con `rho>=3` podría
servir para certificar o refutar el lema técnico de EF-4, pero no resolvería por sí
misma la pregunta fuerte de recuperabilidad. La pregunta con contenido sigue siendo
`liminf T_n>0` (o una variante cuantitativa equivalente), hoy abierta y fuera del
perímetro activo salvo nueva nota firmada.

```text
EF4_ABSOLUTE_MSE_TARGET = TECHNICALLY_MEANINGFUL_BUT_NOT_RECOVERABILITY
EF4_NORMALIZED_INFORMATION_RATIO = NOT_DECIDED
NORMALISED_CHANNEL_LIMINF_T_N = OPEN_SCIENTIFIC_QUESTION_OUT_OF_ACTIVE_SCOPE
```

### 22.3 C1, FORO001-F1 y capacidad real del test finito

La nota firmada del 2026-08-16 levantó la prohibición de ampliar `n` únicamente para
`FORO001-F1`: un test determinista en `(n,rho)=(24,2)`. La propia nota declara que
**no reabre EF-0..EF-8**.

El primer barrido encontró 560 fallos en el dominio abstracto de combinaciones de
filas y columnas. La corrección de realizabilidad mostró que esos testigos no eran
permutaciones compatibles con la prescripción `F_n`. Entre las configuraciones
compatibles que realmente requerían el caso `loss_case` se obtuvo
...
por lo que C1 volvió a `INCONCLUSIVE`: `n=24` no refuta la tricotomía sobre `F_n` y
mucho menos la demuestra.

Hay además una limitación estructural más fuerte. Las escaleras tienen `rho-1`
puntos; con `rho=2` tienen un solo punto, así que una caja no puede contener una
fracción estricta no vacía de una escalera. La contención parcial es imposible para
todo `n` cuando `rho=2`. Por tanto aumentar sólo `n` manteniendo `rho=2` nunca prueba
la rama delicada del análisis de casos.

Si en el futuro se autoriza una auditoría con `rho>=3`, el contrato deberá exigir,
antes de interpretar un barrido negativo, un **control positivo del filtro**: una
configuración compatible conocida que ejercite efectivamente contención parcial y
que el enumerador clasifique como tal. Este requisito se registra aquí; no autoriza
construirlo ni ejecutarlo.

```text
C1_STATUS = INCONCLUSIVE
N24_RHO2 = NO_COMPATIBLE_COUNTEREXAMPLE_BUT_STRUCTURALLY_INCAPABLE_OF_PARTIAL_CONTAINMENT
RHO2_FURTHER_N_SWEEP = NOT_DECISIVE_FOR_PARTIAL_STAIRCASE_BRANCH
FUTURE_RHO_GE_3_POSITIVE_FILTER_CONTROL = REQUIRED_IF_REOPENED
```

### 22.4 Caja de tiempo de R2 y prioridad R1

La nota de reapertura del 2026-07-31 no dejó el vencimiento de R2 como una decisión
posterior. Fijó una regla automática: R2 dispone de dos semanas; si al vencer no está
cerrado, «se marca abierto y se pasa a R1 sin discusión». Ese plazo terminó el
2026-08-14. El vencimiento no convierte R2 en `FAILED` ni en `PROVED`, ni prorroga su
prioridad.

El estado operativo correcto es:

```text
R2_STATUS = OPEN
R2_TIMEBOX = EXPIRED_2026_08_14
R2_DISPOSITION = PASA_A_R1
R2_TIMEOUT_IS_SCIENTIFIC_VERDICT = NO
R1_PRIORITY = ACTIVE
R1_PROGRAM_DEADLINE = 2026_09_11
```

R1 sigue sometido al test de terminado de la nota de reapertura: el manuscrito debe
ser autocontenido y R2 puede aparecer explícitamente como abierto; R2 no puede
bloquear la salida de R1.

### 22.5 Estado canónico después de la reconciliación

Esta sección no reabre Ruta B, EF-4, el canal normalizado ni ninguna ejecución. Solo
resuelve la aparente contradicción entre el cierre registrado en §19 y el trabajo EF
posterior.

```text
RUTA_B_STATUS = CLOSED_UNREPAIRED_BY_PI_2026_08_05
EF0_EF8_STATUS = BACKLOG_OUT_OF_SCOPE
EF4_C1_STATUS = OUT_OF_SCOPE_INCONCLUSIVE
EF4_NEW_EXECUTION = NOT_AUTHORISED
FORO001_F1_N24_RHO2 = EXECUTED_SCOPED_ONLY_NO_COMPATIBLE_COUNTEREXAMPLE
NORMALISED_CHANNEL_LIMINF_T_N = OPEN_SCIENTIFIC_QUESTION_NOT_CURRENT_WORK
R2_STATUS = OPEN_TIMEBOX_EXPIRED_PASA_A_R1
CURRENT_PRIORITY = R1_MANUSCRIPT_LIMITS
CURRENT_PROGRAM_DEADLINE = 2026_09_11
NEW_NUMERICAL_AUTHORIZATION = NONE
NEW_SEEDS_CONSUMED = NONE
SEAL = UNTOUCHED
```
