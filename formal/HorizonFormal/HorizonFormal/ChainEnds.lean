...
/-- Lower set generated by a chain. -/
def IdealGeneratedByChain (c : Nat → P) : Set P :=
  {x : P | ∃ n : Nat, x ≤ c n}

theorem chain_mem_IdealGeneratedByChain (c : Nat → P) (n : Nat) :
    c n ∈ IdealGeneratedByChain c :=
  ⟨n, le_rfl⟩

theorem IdealGeneratedByChain_lower {c : Nat → P} {x y : P}
    (hxy : x ≤ y) (hy : y ∈ IdealGeneratedByChain c) :
    x ∈ IdealGeneratedByChain c := by
  obtain ⟨n, hyn⟩ := hy
  exact ⟨n, le_trans hxy hyn⟩

theorem IdealGeneratedByChain_subset_ideal {I : Order.Ideal P} {c : Nat → P}
    (hmem : ∀ n : Nat, c n ∈ I) :
    IdealGeneratedByChain c ⊆ (I : Set P) := by
  intro x hx
  obtain ⟨n, hxn⟩ := hx
  exact I.lower hxn (hmem n)

theorem ideal_subset_IdealGeneratedByChain {I : Order.Ideal P} {c : Nat → P}
    (hcofinal : ∀ x : P, x ∈ I → ∃ n : Nat, x ≤ c n) :
    (I : Set P) ⊆ IdealGeneratedByChain c := by
  intro x hx
  exact hcofinal x hx

theorem IdealGeneratedByChain_eq_ideal {I : Order.Ideal P} {c : Nat → P}
    (hc : IsCofinalSeqInIdeal I c) :
    IdealGeneratedByChain c = (I : Set P) := by
  apply Set.Subset.antisymm
  · exact IdealGeneratedByChain_subset_ideal hc.1
  · exact ideal_subset_IdealGeneratedByChain hc.2

theorem IdealGeneratedByChain_eq_ideal_of_cofinalChain {I : Order.Ideal P}
    {c : Nat → P} (hc : IsCofinalChainInIdeal I c) :
    IdealGeneratedByChain c = (I : Set P) :=
  IdealGeneratedByChain_eq_ideal hc.2

theorem ChainEventuallyLe_iff_generated_subset {c d : Nat → P} :
    ChainEventuallyLe c d ↔ IdealGeneratedByChain c ⊆ IdealGeneratedByChain d := by
  constructor
  · intro h x hx
    obtain ⟨n, hxn⟩ := hx
    obtain ⟨m, hnm⟩ := h n
    exact ⟨m, le_trans hxn hnm⟩
  · intro h n
    exact h (chain_mem_IdealGeneratedByChain c n)

theorem CofinalChainEquivalent_iff_generated_eq {c d : Nat → P} :
    CofinalChainEquivalent c d ↔ IdealGeneratedByChain c = IdealGeneratedByChain d := by
  constructor
  · intro h
    apply Set.Subset.antisymm
    · exact (ChainEventuallyLe_iff_generated_subset).mp h.1
    · exact (ChainEventuallyLe_iff_generated_subset).mp h.2
  · intro h
    constructor
    · exact (ChainEventuallyLe_iff_generated_subset).mpr (by
        intro x hx
        simpa [h] using hx)
    · exact (ChainEventuallyLe_iff_generated_subset).mpr (by
        intro x hx
